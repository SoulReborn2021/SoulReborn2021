...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
		...
		public void OpenLib(RealStatePtr L)
		{
            ...
            LuaAPI.lua_pushstdcallcfunction(L, StaticLuaCallbacks.XLuaIsNull); 
            LuaAPI.lua_rawset(L, -3);
            LuaAPI.lua_pop(L, 1);

            LuaAPI.lua_createtable(L, 1, 4); 
        ...
        internal void popValuesNoReturn(RealStatePtr L,int oldTop)
        {
            int newTop=LuaAPI.lua_gettop(L);
            if(oldTop==newTop)
            {
        ...
        internal void popValuesNoReturn(RealStatePtr L, int oldTop, Type[] popTypes)
        {
            int newTop=LuaAPI.lua_gettop(L);
            if(oldTop==newTop)
            {
        ...
        public void Get(RealStatePtr L, int index, out decimal val)
        {
            ...
            else if(lua_type == LuaTypes.LUA_TNUMBER)
            {
                ...
                {
                    val = (decimal)LuaAPI.lua_tonumber(L, index); 
                }
            }
            else
            {
